window.renderEventSelector = function () {
    ...
    selector.innerHTML = window.events.map(ev =>
        `<option value="${ev.id}" ${ev.id === window.selectedEventId ? 'selected' : ''}>${ev.name}</option>`
    ).join('');
};
...
window.renderEvents = function () {
    ...
    tbody.innerHTML = window.events.map(ev => `
        <tr class="border-b border-gray-50 hover:bg-gray-50">
            ...
            <td class="px-6 py-3 text-gray-700 font-bold">${ev.name}</td>
            <td class="px-6 py-3 text-gray-600">${ev.date}</td>
            <td class="px-6 py-3">
                <span class="px-2 py-1 rounded text-xs font-bold ${ev.type === 'Mandatory' ? 'bg-red-100 text-red-700' : ev.type === 'Major' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}">${ev.type}</span>
            </td>
        </tr>
    `).join('');
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            const res = await window.apiPost('/api/events', { name, date, type });
            if (res.ok) {
